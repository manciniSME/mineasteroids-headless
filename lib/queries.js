...
export const BOARD_MEMBERS_QUERY = `
  query GetBoardMembers {
    boardMembers(first: 50, where: { orderby: { field: MENU_ORDER, order: ASC } }) {
      nodes {
        title
        boardMemberFields {
          positionTitle
...
export function mapBoardMembers(nodes) {
  return nodes.map((n) => ({
    name: n.title,
    title: n.boardMemberFields.positionTitle,
    photo: n.boardMemberFields.photo?.node?.sourceUrl ?? null,
  }));
}
