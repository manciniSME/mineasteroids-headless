  ...
  return (
    <div style={{ width: '100%', overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', background: '#c1cfe3', minHeight: 480, overflow: 'hidden' }}>
        {slidesError && <p style={{ position: 'relative', zIndex: 6, color: '#900', background: '#fff', padding: 8 }}>Hero slides: {slidesError}</p>}
        {slides.map((slide, n) => (
      ...
      <section style={{ background: '#fff', padding: '48px 0' }}>
        <div style={{ maxWidth: 1140, margin: '0 auto', padding: '0 15px' }}>
          ...
          {postsError && <p style={{ color: BLUE }}>Couldn't reach WPGraphQL: {postsError}</p>}
          ...
          {!postsError && newsItems.length === 0 && <p style={{ color: BLUE }}>No posts yet.</p>}
        </div>
      </section>
